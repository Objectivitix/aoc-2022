...
class Monkey:
    def __init__(
        self, index, starting_items, operation,
        ...
        self.divis_num = divis_num
        self.predicate = lambda x: x % divis_num == 0
        self.true_monkey = true_monkey
        self.false_monkey = false_monkey

    def __repr__(self):
        props_str = ", ".join(f"{key}={value}" for key, value in self.__dict__.items())
...
def turn(monkey, monkeys):
    ...
    while monkey.items:
        ...
        recipient_monkey = monkeys[index]

        print("before: ", monkey.index, worry_level, worry_level % monkey.divis_num, monkey.predicate(worry_level), index)
...
def round(monkeys):
    ...
    for monkey in monkeys:
        turn(monkey, monkeys)
    print([list(monkey.items) for monkey in monkeys])
...
for i, monkey_properties in enumerate(raw):
    ...
    monkey = Monkey(i, starting_items, operation, divis_num, true_monkey, false_monkey)
